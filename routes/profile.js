...
router.post('/', validateToken, upload.single('image'), async (req, res) => {
  try {
    const { name, email, bio, act, add,cat, web, eyear, phone, pone, pnone, ptwo, pntwo, pthree, pnthree, btitle, bdesc,  tm, cp, sms, bm, mjc, cad, tp, fdesc, rev, expen, net } = req.body;
...
router.get('/complete', async (req, res) => {
  try {
    ...
    const requiredFields = [
      'name', 'email', 'bio', 'act','cat', 'add', 'web', 'eyear', 'phone', 'pone', 
      'pnone', 'ptwo', 'pntwo', 'pthree', 'pnthree', 'btitle', 'bdesc',  
...
router.patch('/edit-my-profile', validateToken, upload.single('image'), async (req, res) => {
  try {
    ...
    const { name, email, bio, act, phone,cat, add, web, eyear, pone, pnone, ptwo, pntwo, pthree, pnthree, btitle, bdesc,  tm, cp, sms, bm, mjc, cad, tp, fdesc, rev, expen, net } = req.body;
    ...
    profile.pnone = pnone || profile.pnone;
    profile.ptwo = ptwo || profile.ptwo;
    profile.pntwo = pntwo || profile.pntwo;
    profile.pthree = pthree || profile.pthree;
    profile.pnthree = pnthree || profile.pnthree;
    profile.btitle = btitle || profile.btitle;
